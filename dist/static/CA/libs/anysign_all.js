...
function anysignCommentCore(a, b, c, d) {
	function f() {
		if (I) {
			...
			a.height = I.height / d;
			var g = 1;
			c.mass_word_width / c.mass_word_height <= a.width / a.height ? c.mass_word_width <= a.width && (g = c.mass_word_width / a.width) : c.singleHeight <= a.height && (g = c.mass_word_height / a.height);
			a.width *= g;
			a.height *= g;
			b.scale(1 / d * g, 1 / d * g);
	...
	function h() {
		function b(a) {
			a < e ? (f >= c.mass_words_in_single_line && (u += c.mass_word_height, f = 0), g.drawImage(J[a], c.mass_word_width * f, u, c.mass_word_width, c.mass_word_height), f++, b(a + 1)) : L.push(d.toDataURL("image/png", 1))
		}
	...
	D.innerHTML = v[0].replace(v[0], '\x3cfont color\x3d"' + c.currentEditBarTextColor + '"\x3e$\x26\x3c/font\x3e') + a.substring(1, v.length);
	...
	x.shadowBlur = 5;
	S.onclick = function() {
		p || (n != v.length ? custom_alert("\u8bf7\u5b8c\u6210\u7b7e\u540d", "\u786e\u8ba4") : (h(), signResCallback && signResCallback(c.cid, L[0].substring(22, L[0].length), L[0].substring(22, L[0].length), H, K, N, O), e()))
	};
	T.onclick = function() {
		if (!p) if (0 >= n) alert("\u5df2\u79fb\u9664\u5b8c\uff0c\u8bf7\u91cd\u65b0\u8fdb\u884c\u7b7e\u540d\uff01");
			...
			d.innerHTML = a;
			d.style.color = "black";
			d.innerHTML = a.substring(0, n) + v[n].replace(v[n], '\x3cfont color\x3d"' + c.currentEditBarTextColor + '"\x3e$\x26\x3c/font\x3e') + a.substring(n + 1, v.length)
		}
	};
	U.onclick = function() {
	...
	"ontouchstart" in document.documentElement ? (A.ontouchstart = function(a) {
		...
		if (l) {
			...
			c = a.timeStamp;
			0 !== K || isNaN(c) || (F = c);
			0 < m && u <= A.height && (H = isNaN(c) ? H + (m + "," + u + "," + k + ",0\n") : H + (m + "," + u + "," + k + "," + (c - F) + "\n"), K += 1, m > q && (q = m), m < w && (w = m), u > g && (g = u), u < r && (r = u), y.push({
				x: m,
				y: u
			}), R(), E.push({
				x: m,
			...
			n >= v.length ? (alert("\u5df2\u7b7e\u7f72\u5b8c\u6bd5\uff01"), e.innerHTML = "", p = !1, x.clearRect(0, 0, A.width, A.height)) : P && (P = !1, C = setInterval(function() {
				...
				if (c.isdistinguish && !checkText(v[n])) showProgress("\u6b63\u5728\u8bc6\u522b\uff0c\u8bf7\u7a0d\u5019\u3002\u3002\u3002"), (new OCRObj(c.ocrCapture)).recognition(B, v[n], function(b, m) {
					...
					if (0 == b) {
						if (m) {
							n++;
							n == v.length ? (e.innerHTML = "", D.innerHTML = "\u5df2\u7b7e\u7f72\u5b8c\u6bd5\uff01", D.style.color = "Red", clearInterval(C)) : (c.isShowBgText && (e.innerHTML = v[n]), D.innerHTML = a, D.innerHTML = a.substring(0, n) + v[n].replace(v[n], '\x3cfont color\x3d"' + c.currentEditBarTextColor + '"\x3e$\x26\x3c/font\x3e') + a.substring(n + 1, v.length));
							var h = document.createElement("img");
							h.id = "img" + n;
							h.width = c.mass_word_width;
							h.height = c.mass_word_height;
							if (checkText(v[n - 1])) h.src = u.toDataURL("image/png");
							else {
								var y = q - w,
									k = g - r,
									l = (q + w) / 2,
					...
					} else x.clearRect(0, 0, A.width, A.height), r = w = 9999, g = q = 0, B = [], clearInterval(C);
					...
					if (checkText(v[n - 1])) b.src = u.toDataURL("image/png");
						...
						var m = q - w,
							...
							y = (q + w) / 2,
							k = (g + r) / 2,
							m = 20 > m && 20 > h ? c.mass_word_width : m > h ? m / 2 : h / 2;
		...
		if (l) {
			...
			c = a.timeStamp;
			0 !== K || isNaN(c) || (F = c);
			0 < m && (H = isNaN(c) ? H + (m + "," + u + "," + k + ",0\n") : H + (m + "," + u + "," + k + "," + (c - F) + "\n"), K += 1, u += b, m += d, m > q && (q = m), m < w && (w = m), u > g && (g = u), u < r && (r = u), y.push({
				x: m,
				y: u
			}), R(), E.push({
				x: m,
			...
			n >= v.length ? (alert("\u5df2\u7b7e\u7f72\u5b8c\u6bd5\uff01"), k.innerHTML = "", x.clearRect(0, 0, A.width, A.height), p = !1) : 10 < E.length && (C = setInterval(function() {
				...
				if (c.isdistinguish && !checkText(v[n])) showProgress("\u6b63\u5728\u8bc6\u522b\uff0c\u8bf7\u7a0d\u5019\u3002\u3002\u3002"), (new OCRObj(c.ocrCapture)).recognition(B, v[n], function(b, u) {
					...
					if (0 == b) {
						if (u) {
							n++;
							n == v.length ? (k.innerHTML = "", D.innerHTML = "\u5df2\u7b7e\u7f72\u5b8c\u6bd5\uff01", D.style.color = "Red", clearInterval(C)) : (c.isShowBgText && (k.innerHTML = v[n]), D.innerHTML = a, D.innerHTML = a.substring(0, n) + v[n].replace(v[n], '\x3cfont color\x3d"' + c.currentEditBarTextColor + '"\x3e$\x26\x3c/font\x3e') + a.substring(n + 1, v.length));
							var e = document.createElement("img");
							e.id = "img" + n;
							e.width = c.mass_word_width;
							e.height = c.mass_word_height;
							if (checkText(v[n - 1])) e.src = h.toDataURL("image/png");
							else {
								var m = q - w,
									y = g - r,
									l = (q + w) / 2,
...
function bnpFromString(a, b) {
	...
	for (var d = a.length, f = !1, e = 0; 0 <= --d;) {
		...
		0 > h ? "-" == a.charAt(d) && (f = !0) : (f = !1, 0 == e ? this[this.t++] = h : e + c > this.DB ? (this[this.t - 1] |= (h & (1 << this.DB - e) - 1) << e, this[this.t++] = h >> this.DB - e) : this[this.t - 1] |= h << e, e += c, e >= this.DB && (e -= this.DB))
	}
...
function bnToString(a) {
	...
	if (0 < e--) for (h < this.DB && 0 < (c = this[e] >> h) && (d = !0, f = int2char(c)); 0 <= e;) h < a ? (c = (this[e] & (1 << h) - 1) << a - h, c |= this[--e] >> (h += this.DB - a)) : (c = this[e] >> (h -= a) & b, 0 >= h && (h += this.DB, --e)), 0 < c && (d = !0), d && (f += int2char(c));
	return d ? f : "0"
}
function bnNegate() {
	var a = nbi();
...
function bnpLShiftTo(a, b) {
	var c = a % this.DB,
		...
		h = this.s << c & this.DM,
		k;
	for (k = this.t - 1; 0 <= k; --k) b[k + e + 1] = this[k] >> d | h, h = (this[k] & f) << c;
...
function bnpRShiftTo(a, b) {
	...
	if (c >= this.t) b.t = 0;
		...
		var d = a % this.DB,
			...
			e = (1 << d) - 1;
		b[0] = this[c] >> d;
		for (var h = c + 1; h < this.t; ++h) b[h - c - 1] |= (this[h] & e) << f, b[h - c] = this[h] >> d;
		...
		if (f.t < d.t) null != b && b.fromInt(0), null != c && this.copyTo(c);
			...
			if (0 != f) {
				...
				for (; 0 <= --q;) {
					...
					if ((c[r] += e.am(0, m, c, q, 0, d)) < m) for (e.dlShiftTo(q, g), c.subTo(g, c); c[r] < --m;) c.subTo(g, c)
				}
...
var pubN = "",
	pubE = "10001",
	encCertSN = "‎0990990000012e5010";
	...
	Base64 = {
		...
		encodeUint8Array: function(a) {
			for (var b = 0, c = a.length, d = "", f, e = ""; b < c;) {
				...
				for (var h = 0, k = f.length; h < k; h++) e += String.fromCharCode(f[h]);
				d += e;
				b += 32768;
				e = ""
			}
		...
		decode: function(a) {
			...
			for (var l = new ArrayBuffer(b), p = new Uint8Array(l), b = 0; b < c; b += 4) f = lookup[a.charCodeAt(b)], e = lookup[a.charCodeAt(b + 1)], h = lookup[a.charCodeAt(b + 2)], k = lookup[a.charCodeAt(b + 3)], p[d++] = f << 2 | e >> 4, p[d++] = (e & 15) << 4 | h >> 2, p[d++] = (h & 3) << 6 | k & 63;
			return l
		},
		encode: function(a) {
			var b = "",
				c, d, f, e, h, k, l = 0;
			for (a = Base64._utf8_encode(a); l < a.length;) c = a.charCodeAt(l++), d = a.charCodeAt(l++), f = a.charCodeAt(l++), e = c >> 2, c = (c & 3) << 4 | d >> 4, h = (d & 15) << 2 | f >> 6, k = f & 63, isNaN(d) ? h = k = 64 : isNaN(f) && (k = 64), b = b + this._keyStr.charAt(e) + this._keyStr.charAt(c) + this._keyStr.charAt(h) + this._keyStr.charAt(k);
			return b
		},
...
(function(a) {
	function b() {
		for (var c = !0, b, d = w, g = p.length; d < g && c;) if (b = p[d], void 0 !== b && null !== b) {
			if ("string" === typeof b) {
				var e = k.cloneNode(!1);
				e.text = b;
	...
	function f() {
		var a = arguments.length,
			c, g;
		for (c = 0; c < a; c += 1)"string" === typeof arguments[c] ? (g = r(arguments[c]), g.onload = d(g, p.length), p[p.length] = null, g.send()) : (p[p.length] = arguments[c], b())
	}
	function e() {
	...
	var h = a.document,
		...
		w = 0,
		r, q, g;
	a.XMLHttpRequest && (q = new a.XMLHttpRequest, "withCredentials" in q ? g = function(c) {
		q = new a.XMLHttpRequest;
		q.open("get", c, !0);
		return q
	} : a.XDomainRequest && (g = function(c) {
...
function anysignWebImpl() {
	...
	this._addEvidenceHash = function(a, b, c, d, e) {
		...
		return d == BioType.SOUND_SIGNER_RETELL || d == BioType.SOUND_SIGNER_OTHER ? (g = new SoundEvidence, g.Format = c, g.Hash = b, g.BioType = d, a.Points.CertOID.BioFeature.SoundArray[e] = g, !0) : !1
	};
	this._addEvidence = function(a, b, c, d, e) {
		...
		return d == BioType.SOUND_SIGNER_RETELL || d == BioType.SOUND_SIGNER_OTHER ? (g = new SoundEvidence, g.Format = c, b = sha1Digest(b), g.Hash = "BS64:" + b.toString().toUpperCase(), g.BioType = d, a.Points.CertOID.BioFeature.SoundArray[e] = g, !0) : !1
	};
	...
	this._setTemplate = function(a, b, c, h) {
		if (!d && 10 <= a && 19 >= a) {
			...
			if (null == h || 0 === h.length) return anysign.logger.e("template_serial must not be null or empty"), !1;
			f = c;
			e = sha1DigestBase64(c) + "";
			c = new DataObj;
			c.Cid = a;
			c.Data = b;
			if (10 === a || 13 === a) b = new PDFCrdRule, b.DocFormat = a, b.DocStyleTid = h, c.PDFCrdRule = b;
			h = new DataConfig;
			h.cid = a;
			h.nessesary = !0;
			p[a] = c;
	...
	this._showSignatureDialog = function(a) {
		if (c) {
			if (d) {
				...
				a.innerHTML = f.substring(0, e.titleSpanFromOffset) + g + f.substring(e.titleSpanToOffset + 1, f.length);
	...
	this._showCommentDialog = function(a) {
		if (c) {
			if (d) {
				...
				var e = l[a];
				setSignResCallback(e, function(a, c, d, f, g, h, m) {
					b && (d ? (a = k[a], a.ImageSize = new ImageSize(h, m), a.Image = c, a.SignRule && a.SignRule instanceof SignRule_XYZ && (c = a.SignRule.XYZRule, "dp" === c.Unit ? a.SignRule.XYZRule = {
						Left: c.Left,
						Top: c.Top,
						Right: c.Left + h,
						Bottom: c.Top - m,
		...
		for (b in l) {
			...
			if (e.nessesary) {
				...
				if (isSignOrCom(f.Cid)) {
					if (null == f.Points || null == f.Points.CertOID || null == f.Points.CertOID.BioFeature || null == f.Points.CertOID.BioFeature.Script) return !1
				} else if (null == f.Image) return !1
			}
			isSignOrCom(f.Cid) && null != f.Points && null != f.Image && (a = !0);
			if (!a) return !1
		}
		for (b in w) if (e = w[b], a = p[e.cid], (null == a || null == a.Data) && e.nessesary) return !1;
		return !0
	};
	...
	this._restoreBusinessSession = function(a, b) {
		...
		for (B in g.USignConfigs) {
			...
			for (var F in g.USignObjs) q = g.USignObjs[F].Cid, m == q && (l[m] = g.USignConfigs[B], k[q] = g.USignObjs[F])
		}
		for (B in g.DataConfigs) for (F in m = g.DataConfigs[B].cid, g.DataObjs) q = g.DataObjs[F].Cid, m == q && (w[m] = g.DataConfigs[B], p[q] = g.DataObjs[F]);
		r = g.CachetArray;
		return d = c = !0
	};
	...
	identify_callback, fresh = function() {
		...
		if (isSign) {
			...
			if (1 == a) {
				...
				g.style.height = .65 * a + "px";
				g = document.getElementById("anysign_title");
				g.style.height = .1 * a + "px";
				//document.getElementById("single_scrollbar").style.height = .1 * a + "px";
				g = document.getElementById("btnContainerOuter");
				g.style.height = .1 * a + "px"
			} else 0 == a && (0 == window.orientation || 180 == window.orientation ? (a = .9 * getWindowHeight(), g = getWindowWidth()) : (c = 569 > getWindowWidth() ? .75 * getWindowHeight() : getWindowHeight(), b = getWindowWidth(), a = c, g = b), b = document.getElementById("anysignCanvas"), c = b.getContext("2d"), isDrawn && (d = document.createElement("canvas"), f = d.getContext("2d"), w = p = h = e = l = paste_padding, 0 > (k = minX - l) && (e = minX, k = 0), 0 > (r = minY - l) && (h = minY, r = 0), (q = maxX + l) > b.width && (p = 0, q = b.width), (l = maxY + l) > b.height && (w = 0, l = b.height), e = maxX - minX + e + p, h = maxY - minY + h + w, k = c.getImageData(k, r, q - k, l - r), d.width = e, d.height = h), r = document.getElementById("dialog"), r.style.height = a + "px", r.style.width = g + "px", b.height = a, 10 > b.width && (b.width *= g), b.width < g && (b.width = g), 0 == window.orientation || 180 == window.orientation ? (b.height *= .7, g = document.getElementById("container"), g.style.overflowX = "hidden", g.style.overflowY = "hidden", g.style.height = .7 * a + "px", g = document.getElementById("anysign_title"), g.style.height = .1 * a + "px", g = document.getElementById("btnContainerOuter"), /*g.style.height = .2 * a + "px",*/ a = document.getElementById("btnContainerInner"), a.style.marginBottom = 0) : (b.height *= .6, g = document.getElementById("container"), g.style.overflowX = "hidden", g.style.overflowY = "hidden", g.style.height = .6 * a + "px", g = document.getElementById("anysign_title"), g.style.height = .1 * a + "px", g = document.getElementById("btnContainerOuter"), g.style.height = .3 * a + "px", a = document.getElementById("btnContainerInner"), a.style.marginBottom = g.style.height));
			...
			isDrawn && (a = b.width > e ? b.height >= h ? 1 : b.height / h : b.width / b.height > e / h ? b.height / h : b.width / e, f.putImageData(k, 0, 0), f = document.createElement("canvas"), g = f.getContext("2d"), f.width = e * a, f.height = h * a, g.scale(a, a), g.drawImage(d, 0, 0), c.clearRect(0, 0, b.width, b.height), c.drawImage(f, 0, 0), minY = minX = 0, maxX *= a, maxY *= a);
...
function onload_singleSingScrollAction() {
	...
	function b() {
		d += k;
		d < f ? (e.scrollLeft = d, setTimeout(b, h)) : (d = f, e.scrollLeft = d);
		...
		(new OCRObj(signObjTmp.ocrCapture)).recognition(sbuilder, a, function(a, b) {
			...
			if (0 == a) {
				...
				if (signResCallback) {
					...
					signResCallback(signObjTmp.cid, d, d, signTrack, signTrachPointCount, c.width, c.height)
				}
				document.body.parentNode.style.overflow = "scroll";
				isSign = !1
...
function showMessageBox2(a, b, c, d) {
	...
	a.id = "back";
	a.style.cssText = "font-size:15pt; text-align:center; z-index:7;top:0px;left:0px;position:fixed;background:#666;width:" + d + "px;height:" + c + "px;" + (isIe ? "filter:alpha(opacity\x3d0);" : "opacity:0;");
	c = document.createElement("div");
	c.id = "mesWindow";
	c.innerHTML = "\x3cdiv id\x3d'mesWindowContent'\x3e" + b + "\x3c/div\x3e\x3cdiv id\x3d'mesWindowBottom'\x3e\x3cinput id\x3d'mesWindowBtnOk' type\x3d'button' onclick\x3d'closeWindow();'value\x3d'\u786e\u8ba4' /\x3e\x3c/div\x3e";
	a.appendChild(c);
...
function crc32(a, b, c) {
	...
	var a = function() {
			...
			for (var h = f * (4 * b + 1), k = 0; k < f; k++) a.splice(k * (4 * b + 1), 0, 0);
			b = Math.ceil(h / 32768);
